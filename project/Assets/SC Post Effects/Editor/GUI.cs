...
namespace SCPE
{
    public class SCPE_GUI : Editor
    {
        ...
        {
#if !UNITY_2020_2_OR_NEWER && URP //Supports the ConfigureInput function, which prompts the RP to render the depth pre-pass when required
        ...
        public static void DisplayTextureOverrideWarning(bool isOverridden)
        {
            if (!isOverridden)
            {
                EditorGUILayout.Space();
                EditorGUILayout.HelpBox("Texture parameters always need to be overridden. It will otherwise default to a black texture in a build.", MessageType.Error);
            }
        }
        
        //Dummy overload, when using PPS
        public static void DisplaySetupWarning<T>() { DisplayDualInstallationError(); }
        
        //Dummy overload, for v2.3.3
        public static void DisplaySetupWarning<T>(ref bool state, SerializedObject so, bool compatible2D = true) { }
        
        public static void DisplaySetupWarning<T>(ref bool state, bool compatible2D = true)
        {
#if URP
            DisplayRenderGraphWarning();
            ...
            bool is2D = forwardRenderer.GetType() != typeof(UniversalRendererData);
            ...
            DisplayDualInstallationError();
        }

        public static void DisplayRenderGraphWarning()
        {
            ...
            if (GraphicsSettings.GetRenderPipelineSettings<RenderGraphSettings>().enableRenderCompatibilityMode == false)
            {
                EditorGUILayout.HelpBox("Using Render Graph in Unity 6+ is not supported." +
                                        "\n\nEnable Backwards / Compatibility mode under Project Settings > Graphics > URP asset > Render Graph. (Unity 6.4+ no longer allows changing this from script.)", MessageType.Error);
                
                GUILayout.Space(-32);
                using (new EditorGUILayout.HorizontalScope())
                {
                    GUILayout.FlexibleSpace();
                    if (GUILayout.Button(new GUIContent("Open Graphics Settings", EditorGUIUtility.IconContent("d_tab_next").image), GUILayout.Width(170)))
                    {
        ...
        private static void DisplayDualInstallationError()
        {
            #if URP && PPS
			EditorGUILayout.HelpBox("Post Processing package and URP cannot both be installed."+
"               \n\nUninstall the package not appropriate for the chosen render pipeline...", MessageType.Error);
            #endif
        }
        ...
        private static bool IsVREnabled()
        {
            ...
            return buildTargetSettings.SettingsForBuildTarget(buildTargetGroup).AssignedSettings.activeLoaders.Count > 0;
            #else
        ...
        public static void DisplayVRWarning(bool requiresStereoMatrices = false) //bool parameter left to ensure proper upgrade to 2.3.0+
        {
            ...
            EditorGUILayout.HelpBox("Not compatible with Single-Pass Instanced rendering (VR)", MessageType.Error);
        ...
        public static class Installation
        {
            ...
            public static void DrawAssetVersion(bool odd = false)
            {
                ...
                if (!AssetVersionCheck.IS_UPDATED)
                {
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        if (SCPE_GUI.DrawActionBox("Update to " + AssetVersionCheck.latestVersion, EditorGUIUtility.IconContent("BuildSettings.Standalone.Small").image))
                        {
            ...
            public static void DrawUnityVersion(bool odd = false)
            {
                ...
                versionText = (UnityVersionCheck.COMPATIBLE) ? "Compatible" : "Unsupported";
                versionText = (UnityVersionCheck.BETA) ? "Alpha or Beta" : versionText;
                SCPE_GUI.Status versionStatus;
                ...
                #if UNITY_6000_0_OR_NEWER
                EditorGUILayout.HelpBox("This asset does not support Unity 6 for the Universal Render Pipeline. Expect console warnings...", MessageType.Warning);
                #endif
                
                if(UnityVersionCheck.COMPATIBLE == false) EditorGUILayout.HelpBox("Support for this version has been retired. Requires Unity " + SCPE.MIN_UNITY_VERSION + " or newer.",  MessageType.Warning);

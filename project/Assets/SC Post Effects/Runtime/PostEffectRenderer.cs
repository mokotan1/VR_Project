...
#if UNITY_6000_0_OR_NEWER
        , ISCPEUnsafePassExecutor
#endif
    {
        public bool render;

#if UNITY_6000_0_OR_NEWER
        /// <summary>Snapshot from <see cref="Setup"/>; used when <c>RecordRenderGraph</c> runs (same frame).</summary>
        protected RenderingData scpeRenderingData;

        CommandBuffer scpeUnsafeNativeCmd;
#endif
        ...
        private static Material BlitMaterial
        {
            get
            {
                if (!_BlitMaterial)
                {
                    ...
                    Shader blitShader = shaders.coreBlitPS;
                    #else
        ...
        {
#if UNITY_6000_0_OR_NEWER
            scpeRenderingData = renderingData;
#endif
            render = true;
        ...
        /// <summary>
        /// URP 17 / Unity 6 removed <c>OnCameraSetup</c> from <see cref="ScriptableRenderPass"/>; legacy path still overrides it.
        /// Render-graph path calls this from <see cref="ISCPEUnsafePassExecutor.RunUnsafeRenderGraphPass"/> (after <see cref="Setup"/>).
        /// </summary>
        protected virtual void OnSCPECameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
        {
        ...
        {
            OnSCPECameraSetup(cmd, ref renderingData);
        }
        ...
        {
            ApplySCPECameraDescriptor(cmd, cameraTextureDescriptor);
        }
        #pragma warning restore CS0618
        #pragma warning restore CS0672
#endif
        ...
        /// <summary>
        /// Copies the camera target descriptor (MSAA cleared) and runs <see cref="ConfigurePass"/>.
        /// </summary>
        protected void ApplySCPECameraDescriptor(CommandBuffer cmd, RenderTextureDescriptor cameraTextureDescriptor)
        {
...
#if UNITY_6000_0_OR_NEWER
        void SCPE_RenderGraphApplySetup(CommandBuffer cmd, ref RenderingData renderingData)
        {
            OnSCPECameraSetup(cmd, ref renderingData);
            ApplySCPECameraDescriptor(cmd, renderingData.cameraData.cameraTargetDescriptor);
        }
#endif
        ...
        {
#if UNITY_6000_0_OR_NEWER
            if (scpeUnsafeNativeCmd != null)
                return scpeUnsafeNativeCmd;
#endif
            #if UNITY_2022_1_OR_NEWER
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            using (var builder = renderGraph.AddUnsafePass<SCPERGPassPayload>(ProfilerTag, out var passData, profilingSampler))
            {
                passData.Executor = this;
                ...
                if (requiresDepthNormals && !reconstructDepthNormals && resourceData.cameraNormalsTexture.IsValid())
                    builder.UseTexture(resourceData.cameraNormalsTexture, AccessFlags.Read);

                builder.SetRenderFunc(static (SCPERGPassPayload data, UnsafeGraphContext ctx) =>
                {
                    data.Executor.RunUnsafeRenderGraphPass(ctx);
                });
            }
        }

        void ISCPEUnsafePassExecutor.RunUnsafeRenderGraphPass(UnsafeGraphContext ctx)
        {
            scpeUnsafeNativeCmd = CommandBufferHelpers.GetNativeCommandBuffer(ctx.cmd);
            try
            {
                ScriptableRenderContext sctx = SCPERGBridge.GetScriptableRenderContext(ctx);
                ref RenderingData rd = ref scpeRenderingData;
                SCPE_RenderGraphApplySetup(scpeUnsafeNativeCmd, ref rd);
                ExecuteSCPE(sctx, ref rd);
            }
            finally
            {
                scpeUnsafeNativeCmd = null;
            }
        }
#endif

        /// <summary>
        /// Effect implementation. On Unity 6+ URP this is driven from <c>RecordRenderGraph</c> (unsafe pass).
        /// </summary>
        protected virtual void ExecuteSCPE(ScriptableRenderContext context, ref RenderingData renderingData)
        {
        }
        ...
        {
            ExecuteSCPE(context, ref renderingData);
        }
        #pragma warning restore CS0618
        #pragma warning restore CS0672
#endif
        ...
        protected void FinalBlit(ScriptableRenderPass pass, ScriptableRenderContext context, CommandBuffer cmd, RenderingData renderingData, int passIndex)
        {
            ...
            if (scpeUnsafeNativeCmd != null)
            {
                // Native buffer is owned/submitted by the render graph; do not pool-release or ExecuteCommandBuffer again.
                return;
            }
#endif
